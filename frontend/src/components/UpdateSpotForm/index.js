...
const UpdateSpotForm = () => {
    ...
    const onSubmit = async (e) => {
        ...
        if (!Object.values(errors).length) {
            const updatedSpot = await dispatch(spotActions.editSpot(spotInfo, spotId))
                .catch(async (res) => {
            ...
            if (updatedSpot) {
                await dispatch(spotActions.postImage(+updatedSpot.id, previewImgInfo))
                if (image1) {
                    await dispatch(spotActions.postImage(+updatedSpot.id, Img1Info))
                }
                if (image2) {
                    await dispatch(spotActions.postImage(+updatedSpot.id, Img2Info))
                }
                if (image3) {
                    await dispatch(spotActions.postImage(+updatedSpot.id, Img3Info))
                }
                if (image4) {
